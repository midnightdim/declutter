        ...
        for f in tag_files:
            ...
            if os.path.isfile(filename) or os.path.isdir(filename):
                try:
                    ...
                    # TBD this doesn't look very safe
                    logging.warning(
                        "Removing tag file cause it seems to be corrupt: " + fullname)
                    os.remove(fullname)
            else:
                logging.warning(
                    "Deleting this tag file since no corresponding file was found: " + fullname)
                os.remove(fullname)
    if recursive:
...
def set_tags_sc(filename, tags):
    if Path(filename).is_file() or Path(filename).is_dir():
        if set(get_tags_sc(filename)) != set(tags):
            ...
            try:
                # had to use w+ instead of w because of restrictions with hidden files
