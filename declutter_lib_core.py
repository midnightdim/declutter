...
def apply_rule(rule, dryrun=False):
    report = {'copied': 0, 'moved': 0, 'moved to subfolder': 0,
              'deleted': 0, 'trashed': 0, 'renamed': 0}
    details = []
    if rule['enabled']:
...
def apply_all_rules(settings):
    ...
    for rule in settings['rules']:  # TBD this doesn't respect the order by id
        rule_report, rule_details = apply_rule(
            rule, settings['dryrun'])  # TBD vN doesn't look optimal
        # print(rule_report)
