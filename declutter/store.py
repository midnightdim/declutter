...
def add_recent_folder(path: str, max_items: int = 15):
    with get_conn() as conn:
        ...
        conn.execute("INSERT INTO recent_folders(path, list_order) VALUES (?,?)", (path, 1))
    ...
    with get_conn() as conn:
        ...
        for rr in rule_rows:
            ...
            folders = [r["folder"] for r in conn.execute(
                "SELECT folder FROM rule_folders WHERE rule_id=? ORDER BY id", (rid,)
            ).fetchall()]
            cond_rows = conn.execute(
                "SELECT type, payload FROM rule_conditions WHERE rule_id=? ORDER BY id", (rid,)
            ).fetchall()
...
def replace_rules(rules: List[Dict[str, Any]]):
    with get_conn() as conn:
        ...
        for rule in rules:
            ...
            rid = conn.execute("SELECT id FROM rules ORDER BY id DESC LIMIT 1").fetchone()["id"]

            for folder in rule.get('folders', []):
                conn.execute("INSERT INTO rule_folders(rule_id, folder) VALUES (?,?)", (rid, folder))
            ...
            for t in rule.get('tags', []):
                ...
                conn.execute("INSERT INTO rule_tags(rule_id, tag_id) VALUES (?,?)", (rid, tag_id))
        conn.commit()
    ...
    if 'recent_folders' in settings and isinstance(settings['recent_folders'], list):
        with get_conn() as conn:
            ...
            for p in settings['recent_folders']:
                conn.execute("INSERT INTO recent_folders(path, list_order) VALUES (?,?)", (p, order))
                order += 1
            conn.commit()
