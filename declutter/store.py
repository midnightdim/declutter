...
def list_file_types() -> List[Dict[str, Any]]:
    with get_conn() as conn:
        rows = conn.execute("SELECT id, name, patterns, list_order FROM file_types ORDER BY list_order, id").fetchall()
...
def replace_file_types(items: List[Dict[str, Any]]):
    with get_conn() as conn:
        ...
        for it in items:
            ...
            conn.execute("INSERT INTO file_types(name, patterns, list_order) VALUES (?,?,?)", (name, patterns, order))
            order += 1
        conn.commit()
...
def list_recent_folders() -> List[str]:
    with get_conn() as conn:
        rows = conn.execute("SELECT path FROM recent_folders ORDER BY list_order, id").fetchall()
...
def add_recent_folder(path: str):
    ...
    with get_conn() as conn:
        ...
        conn.execute("INSERT INTO recent_folders(path, added_at, list_order) VALUES (?,?,?)", (path, now, 1))
        rows = conn.execute("SELECT id FROM recent_folders ORDER BY list_order, id").fetchall()
        if len(rows) > 15:
            ids_to_delete = [r["id"] for r in rows[15:]]
            conn.executemany("DELETE FROM recent_folders WHERE id=?", [(i,) for i in ids_to_delete])
        conn.commit()
    ...
    with get_conn() as conn:
        ...
        for rr in rule_rows:
            ...
            folders = [r["folder"] for r in conn.execute(
                "SELECT folder FROM rule_folders WHERE rule_id=? ORDER BY id", (rule_id,)
            ).fetchall()]
            cond_rows = conn.execute(
                "SELECT type, payload FROM rule_conditions WHERE rule_id=? ORDER BY id", (rule_id,)
            ).fetchall()
            conditions = []
            for cr in cond_rows:
                try:
            ...
            # join to tags to get tag names
            tag_rows = conn.execute(
                "SELECT t.name FROM rule_tags rt JOIN tags t ON rt.tag_id = t.id WHERE rt.rule_id=? ORDER BY rt.id",
                (rule_id,)
            ).fetchall()
...
def _ensure_tag_and_get_id(conn: sqlite3.Connection, tag_name: str) -> int:
    ...
    if row:
        return row["id"]
    # append to default group 1
    max_row = conn.execute("SELECT COALESCE(MAX(list_order),0) AS m FROM tags WHERE group_id=1").fetchone()
...
def save_settings(_: Optional[str], settings: Dict[str, Any]):
    for k in PRIMITIVE_KEYS.keys():
        if k in settings:
            set_setting(k, settings[k])

    if 'file_types' in settings and isinstance(settings['file_types'], dict):
        items = [{"name": name, "patterns": pat} for name, pat in settings['file_types'].items()]
    ...
    if 'recent_folders' in settings and isinstance(settings['recent_folders'], list):
        with get_conn() as conn:
            ...
            for p in settings['recent_folders']:
                conn.execute(
                    "INSERT INTO recent_folders(path, added_at, list_order) VALUES (?,?,?)",
                    (p, now, order)
                )
                order += 1
            conn.commit()
